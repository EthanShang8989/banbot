...
// RunHistFeeders runs historical feeders with the new Batch interface
...
func RunHistFeeders(makeFeeders func() []IHistFeeder, versions chan int, pBar *utils.PrgBar) *errs.Error {
	...
	for {
		...
		if ver > oldVer || firstInit {
			feeders = makeFeeders()
			feeders = SortHistFeeders(feeders)
...
// SortHistFeeders sorts feeders by their next event time
func SortHistFeeders(feeders []IHistFeeder) []IHistFeeder {
...
func (p *HistProviderV2) LoopMain() *errs.Error {
	...
	return RunHistFeeders(makeFeeders, p.dirtyVers, prgBar)
}
